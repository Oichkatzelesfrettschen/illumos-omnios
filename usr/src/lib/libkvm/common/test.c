...
struct nlist nl[] = {{"free"},     {"fragtbl"}, {"freemem"}, {"allthreads"},
                     {"nbuckets"}, {"cputype"}, {0}};

/**
 * @brief Entry point for the libkvm test program.
 *
 * Opens the kernel image and iterates through process structures to
 * demonstrate libkvm functionality.
 *
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  printf("pid %d:: %d args; %d envs; %d chars (%p - %p)\n", getpid(), argc,
         envc, ccnt, &argv[0], *envp + strlen(*envp));
  ...
  while ((proc = tst_nextproc()) != NULL) {
    struct pid pid;
    if (kvm_read(cookie, (uintptr_t)proc->p_pidp, &pid, sizeof(pid)) !=
        sizeof(pid)) {
...
/**
 * @brief Open the kernel image and prepare a libkvm handle.
 *
 * @param namelist  Path to the kernel symbol file or NULL for live kernel.
 * @param corefile  Path to the crash dump or NULL for live kernel.
 * @param swapfile  Path to the swap file when analysing dumps.
 * @param flag      Open flags passed to libkvm.
 */
void tst_open(char *namelist, char *corefile, char *swapfile, int flag) {
  printf("kvm_open(%s, %s, %s, %s)\n",
         (namelist == NULL) ? "LIVE_KERNEL" : namelist,
         (corefile == NULL) ? "LIVE_KERNEL" : corefile,
         (swapfile == NULL) ? ((corefile == NULL) ? "LIVE_KERNEL" : "(none)")
                            : swapfile,
         (flag == O_RDONLY) ? "O_RDONLY"
                            : ((flag == O_RDWR) ? "O_RDWR" : "???"));

  if ((cookie = kvm_open(namelist, corefile, swapfile, flag, "libkvm test")) ==
      NULL)
    printf("ERROR: kvm_open returned %p\n", cookie);
}

/**
 * @brief Close the libkvm handle.
 */
void tst_close(void) {
  int i;
...
/**
 * @brief Resolve a list of kernel symbols using libkvm.
 *
 * @param nl Array of nlist structures to be resolved. The array must be
 *           terminated with a zero name entry.
 */
void tst_nlist(struct nlist nl[]) {
  int i;
  char *t, *s;
...
/**
 * @brief Read bytes from kernel memory.
 *
 * @param addr   Kernel virtual address to read from.
 * @param buf    Destination buffer for the data.
 * @param nbytes Number of bytes to read.
 * @return Number of bytes read.
 */
ssize_t tst_read(uintptr_t addr, void *buf, size_t nbytes) {
  ssize_t e;
  int i;
  char *b;
...
/**
 * @brief Write bytes to kernel memory and verify the result.
 *
 * After writing, the memory is read back to ensure the operation
 * succeeded.
 *
 * @param addr   Kernel virtual address to write to.
 * @param buf    Buffer containing bytes to write.
 * @param nbytes Number of bytes to write.
 * @return Number of bytes written.
 */
ssize_t tst_write(uintptr_t addr, void *buf, size_t nbytes) {
  ssize_t e;
  ssize_t i;
  void *b;
...
/**
 * @brief Retrieve a process structure for a given PID.
 *
 * @param pid Process ID to look up.
 * @return Pointer to the process structure or NULL on error.
 */
struct proc *tst_getproc(pid_t pid) {
  struct proc *proc;
  struct pid pidbuf;
  ...
  if (kvm_read(cookie, (uintptr_t)proc->p_pidp, &pidbuf, sizeof(pidbuf)) !=
      sizeof(pidbuf)) {
    printf("ERROR: couldn't get pid\n");
    return (proc);
  }
...
/**
 * @brief Retrieve the next process in the global process list.
 *
 * @return Pointer to the next process structure or NULL when no more
...
/**
 * @brief Reset the internal process iterator used by libkvm.
 *
 * @return 0 on success, otherwise an error code from libkvm.
 */
int tst_setproc(void) {
  int i;
...
/**
 * @brief Obtain the user structure for a given process.
 *
 * @param proc Pointer to the process structure.
 * @return Pointer to the user structure or NULL on error.
 */
struct user *tst_getu(struct proc *proc) {
  register int e;
  struct proc tp;
  struct user *u;
  struct pid pidbuf;
...
/**
 * @brief Print a string replacing non-printable characters with spaces.
 *
 * @param s Input string to print safely.
 */
...
/**
 * @brief Retrieve command line arguments and environment for a process.
 *
 * @param proc Process structure.
 * @param u    Associated user structure.
 * @return 0 on success, otherwise an error code.
 */
int tst_getcmd(struct proc *proc, struct user *u) {
  char **arg;
  char **env;
  int i;
  char **p;
  ...
  if (kvm_kread(cookie, (uintptr_t)proc->p_pidp, &pidbuf, sizeof(pidbuf)) !=
      sizeof(pidbuf)) {
    printf("ERROR: couldn't get pid\n");
    return (-1);
  }
...
/**
 * @brief Iterate over kernel threads to display stack information.
 */
void tst_segkp(void) {
  kthread_t t;
  caddr_t tp, alltp;
  ...
  if (kvm_read(cookie, nl[3].n_value, &alltp, sizeof(alltp)) != sizeof(alltp)) {
  ...
  for (tp = alltp; tp; tp = (caddr_t)(t.t_next)) {
    if (kvm_read(cookie, (uintptr_t)tp, &t, sizeof(t)) != sizeof(t)) {
    ...
    printf("\tstk 0x%p sp 0x%lx tid %d next 0x%p prev 0x%p\n", tp, t.t_stk,
           t.t_pcb.val[1], t.t_tid, t.t_next, t.t_prev);

    if (kvm_read(cookie, t.t_pcb.val[1] + STACK_BIAS, stk, sizeof(stk)) !=
        sizeof(stk)) {
